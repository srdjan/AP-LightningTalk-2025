  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('```')) {
      if (!inCode) {
        ...
        out += `</code></pre><button class="copy-btn" data-copy>Copy</button></div>`;
        inCode = false;
      }
      continue;
    }
...
function updateSlides() {
  ...
  state.htmlSections.forEach((sec, i) => {
    ...
    fr.forEach((el, j) => {
      el.style.opacity = j < state.fragIdx[i] ? '1' : '0.15';
    });
  });
  announce();
}
...
function announce() {
  ...
  announcer.textContent = `${title}, slide ${state.current + 1} of ${state.htmlSections.length}`;
...
function showNotes() {
  const txt = state.notes[state.current] || (state.htmlSections[state.current]?.querySelector('h1,h2')?.textContent || '');
